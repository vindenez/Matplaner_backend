...
public interface RecipeRepository extends JpaRepository<Recipe, String>, JpaSpecificationExecutor<Recipe> {
    ...
    @Query(value = "SELECT * FROM recipes r WHERE (levenshtein(r.name, :query) <= :maxDistance OR EXISTS (SELECT 1 FROM unnest(r.tags) tag WHERE levenshtein(tag, :query) <= :maxDistance)) AND r.current_price BETWEEN :minPrice AND :maxPrice",
            nativeQuery = true)
    List<Recipe> findByNameOrTagsFuzzyWithPriceFilter(
